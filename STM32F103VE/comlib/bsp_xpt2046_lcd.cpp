...
{
	// 模拟SPI GPIO初始化
	XPT2046_SPI_CLK.setMode(GPIOMode::OUT_PushPull, GPIOSpeed::Atmost_10MHz);
	XPT2046_SPI_MOSI.setMode(GPIOMode::OUT_PushPull, GPIOSpeed::Atmost_10MHz);
	XPT2046_SPI_CS.setMode(GPIOMode::OUT_PushPull, GPIOSpeed::Atmost_10MHz);
	XPT2046_SPI_MISO.setMode(GPIOMode::IN_Pull);
	XPT2046_SPI_MISO.setPull(true);//上拉输入
	// 拉低片选，选择XPT2046
	XPT2046_SPI_CS = false;						
	//触摸屏触摸信号指示引脚，不使用中断
	XPT2046_PENIRQ_GPIO.setMode(GPIOMode::IN_Pull);
	XPT2046_PENIRQ_GPIO.setPull(true);
}
...
static void XPT2046_WriteCMD ( uint8_t ucCmd ) 
{
	XPT2046_SPI_MOSI = false;//== XPT2046_MOSI_0();	
	
	XPT2046_SPI_CLK = false;//== XPT2046_CLK_LOW();
	for0 ( i , 8 ) 
	{
		if( ( ucCmd >> ( 7 - i ) ) & 0x01 )
			XPT2046_SPI_MOSI = true;
		else XPT2046_SPI_MOSI = false;

		XPT2046_DelayUS ( 5 );
		XPT2046_SPI_CLK = true;//== XPT2046_CLK_HIGH();
		XPT2046_DelayUS ( 5 );
		XPT2046_SPI_CLK = false;//== XPT2046_CLK_LOW();
	}
...
static uint8_t XPT2046_ReadAdc_Smooth_XY ( strType_XPT2046_Coordinate * pScreenCoordinate )
{
	...
	}	while ( ( XPT2046_PENIRQ_GPIO == XPT2046_PENIRQ_ActiveLevel ) && ( ucCount < 10 ) );//用户点击触摸屏时即TP_INT_IN信号为低 并且 ucCount<10
	
	
	/*如果触笔弹起*/
	if ( XPT2046_PENIRQ_GPIO != XPT2046_PENIRQ_ActiveLevel )
...
{ 
	...
	switch(touch_state)
	{
		case XPT2046_STATE_RELEASE:	
			if(XPT2046_PENIRQ_GPIO == XPT2046_PENIRQ_ActiveLevel) //第一次出现触摸信号
			{
		...
		case XPT2046_STATE_PRESSED:	
				if(XPT2046_PENIRQ_GPIO == XPT2046_PENIRQ_ActiveLevel)		//触摸持续按下
				{
					touch_state = XPT2046_STATE_PRESSED;
					detectResult = TOUCH_PRESSED;
				}
